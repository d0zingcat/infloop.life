...
另外关于上面的一些启动命令，默认都往`~/.bash_profile`和`~/.profile`写了，都没有往`~/.bashrc`里面写，但其实每当新开一个terminal的时候调用的是`~/.bashrc`中的命令，对于我们而言最方便的是写到`~/.bashrc`中。关于这三个差别可以参考：
